...
(function(){
  ...
  function storeGet(key, fallback){
    try{
      ...
      if(key === LS_KEY && fs?.reports && typeof fs.reports.getFavorites === 'function'){
        const v = fs.reports.getFavorites();
  ...
  function loadArr(key){
    try{
      const fs = window.FinStore;
      if(key === 'fin_cp_rows_v1' && fs?.cp && typeof fs.cp.getRows === 'function'){
        const arr = fs.cp.getRows();
        return Array.isArray(arr) ? arr : [];
      }
      if(key === 'fin_cr_rows_v1' && fs?.cr && typeof fs.cr.getRows === 'function'){
        const arr = fs.cr.getRows();
        return Array.isArray(arr) ? arr : [];
      }
    }catch(_){}
  ...
  function closeModal(){
    ...
    // Mantém o modal visível até concluir a transição de saída.
  ...
  function buildKpiCards(kpis){
    ...
    printKpis.innerHTML = kpis.map(k => {
      const tone = (k && k.tone) ? String(k.tone) : toneClassFromText(k?.label, k?.value, '');
      return `<div class="fr-kpi ${escapeHtml(tone)}"><div class="fr-kpi__k">${escapeHtml(k.label)}</div><div class="fr-kpi__v">${escapeHtml(k.value)}</div></div>`;
    }).join('');
  }

  function toneClassFromText(label, value, fallback){
    ...
    if(/(entrada|entradas|receber|recebimento|receita|\bcr\b)/.test(text)) return 'is-in';
    if(/(saida|saídas|saidas|pagar|despesa|custo|\bcp\b)/.test(text)) return 'is-out';
    return '';
  }

  function decorateReportTones(res){
    if(!res || typeof res !== 'object') return res;
    if(Array.isArray(res.kpis)){
      res.kpis = res.kpis.map(k => {
        if(!k || typeof k !== 'object') return k;
        if(k.tone) return k;
        return Object.assign({}, k, { tone: toneClassFromText(k.label, k.value, '') });
      });
    }
    if(Array.isArray(res.chartSum)){
      res.chartSum = res.chartSum.map(r => {
        if(!r || typeof r !== 'object') return r;
        if(r.tone) return r;
        return Object.assign({}, r, { tone: toneClassFromText(r.label, r.value, '') });
      });
    }
    return res;
  }
  ...
  function renderTable(columns, rows, totalRow){
    ...
    const totalHtml = totalRow ? `<tr class="is-total">${cols.map((c, idx) => {
      ...
      const tone = toneClassFromText(c.label || '', v, totalRow.label || '');
      const fullCls = [cls, tone].filter(Boolean).join(' ');
      return `<td class="${fullCls}">${escapeHtml(v)}</td>`;
    }).join('')}</tr>` : '';

    tbodyEl.innerHTML = (rs.length ? bodyRows : emptyRow) + totalHtml;
  }

  function renderResumoSplit(split){
    if(!theadEl || !tbodyEl) return;

    const cpRows = Array.isArray(split?.cpRows) ? split.cpRows : [];
    const crRows = Array.isArray(split?.crRows) ? split.crRows : [];
    const totalCP = Number(split?.totalCP || 0);
    const totalCR = Number(split?.totalCR || 0);
    const saldo = Number(split?.saldo || 0);

    const esc = (s) => escapeHtml(String(s ?? ''));
    const statusTxt = (s) => (String(s || '').toLowerCase() === 'done' ? 'Concluído' : 'Aberto');
    const dt = (iso) => iso ? toBRddmmyyyy(iso) : '—';

    function bodyRows(list){
      if(!list.length){
        return `<tr><td colspan="5">Nenhum item no período.</td></tr>`;
      }
      return list.map(r => `
        <tr>
          <td>${esc(r.descricao || '—')}</td>
          <td>${esc(r.detalhe || '—')}</td>
          <td class="t-center">${esc(dt(r.data))}</td>
          <td class="t-center">${esc(statusTxt(r.status))}</td>
          <td class="t-right">${esc(moneyBR(r.valor))}</td>
        </tr>
      `).join('');
    }

    const saldoClass = saldo > 0 ? 'is-pos' : (saldo < 0 ? 'is-neg' : 'is-neutral');

    theadEl.innerHTML = '';
    tbodyEl.innerHTML = `
      <tr>
        <td colspan="1" class="fr-resumo-split-cell">
          <div class="fr-resumo-split">
            <div class="fr-resumo-box is-cr">
              ...
              <div class="fr-resumo-box__total is-cr">Total CR: ${esc(moneyBR(totalCR))}</div>
            </div>
            ...
            <div class="fr-resumo-box is-cp">
              ...
              <div class="fr-resumo-box__total is-cp">Total CP: ${esc(moneyBR(totalCP))}</div>
            </div>
          </div>

          <div class="fr-resumo-saldo ${saldoClass}">
            <span>Saldo do período</span>
            <strong>${esc(moneyBR(saldo))}</strong>
          </div>
        </td>
      </tr>
    `;
  ...
  function renderChart(chartCfg, sumRows){
    ...
    if(chartSumEl && Array.isArray(sumRows) && sumRows.length){
      chartSumEl.innerHTML = sumRows.slice(0, CHART_TOP_N).map(r => {
        const tone = (r && r.tone) ? String(r.tone) : toneClassFromText(r.label, r.value, '');
        return `
          <div class="fr-print__sumrow ${tone}">
  ...
  function execReport(rid){
    ...
    if(rid === 'rep_resumo_mes'){
      ...
      const entradasList = crIn
        .map(r => ({
          descricao: String(r.cliente || '—'),
          detalhe: String(r.processo || r.forma || '—'),
          data: r.data,
          status: r.status || 'open',
          valor: asVal(r),
        }))
        .sort((a,b) => safeTime(a.data) - safeTime(b.data));

      const saidasList = cpIn
        .map(p => ({
          descricao: String(p.conta || '—'),
          detalhe: String([p.imovel || '', p.categoria || ''].filter(Boolean).join(' • ') || '—'),
          data: p.data,
      ...
      columns = [
        ...
        { label:'Data', value:r => r.data ? toBRddmmyyyy(r.data) : '—', align:'center' },
        { label:'Status', value:r => r.status === 'done' ? 'Concluído' : 'Aberto', align:'center' },
        { label:'Valor', value:r => moneyBR(r.valor), align:'right' },
      ];
      rows = entradasList.map(r => ({ ...r, tipo:'CR' })).concat(saidasList.map(r => ({ ...r, tipo:'CP' })));

      chart = window.Chart ? {
        type: 'doughnut',
        data: {
          labels: ['Total CP', 'Total CR'],
          datasets: [{
            data: [saidas, entradas],
            backgroundColor: ['rgba(239,68,68,.72)', 'rgba(16,185,129,.72)'],
            borderWidth: 0,
          }],
        },
        options: {
          ...
          plugins: {
            legend: { position: 'bottom' },
            tooltip: { callbacks: { label: (ctx) => `${ctx.label}: ${moneyBR(ctx.raw)}` } },
          },
          cutout: '56%',
        }
      } : null;

      const totalChart = entradas + saidas;
      chartSum = [
        { label:'Total CP', value: moneyBR(saidas), pct: totalChart>0 ? (pctOf(saidas,totalChart)).toFixed(1).replace('.',',')+'%' : '0,0%' },
        { label:'Total CR', value: moneyBR(entradas), pct: totalChart>0 ? (pctOf(entradas,totalChart)).toFixed(1).replace('.',',')+'%' : '0,0%' },
        { label:'Saldo', value: moneyBR(saldo), pct:'—' },
      ];
  ...
  function renderExecResult(res){
    ...
    if(res.layout === 'cpcr_split') renderResumoSplit(res.splitData || null);
    else renderTable(res.columns, res.rows, res.totalRow);
  ...
  if(modalPrint){
    modalPrint.addEventListener('click', () => {
      ...
      if(lastExec.totalRow && cols.length){
        ...
        totalTones = total.map((v, idx) => toneClassFromText(head[idx] || '', v, total[0] || ''));
      }

      const corp = (window.__FR_MOCK__ && window.__FR_MOCK__.corp) ? window.__FR_MOCK__.corp : {};

      function buildPreviewPrefs(companyData){
        const out = {
          theme: {
            accentPreset: 'blue',
          },
        ...
        try {
          if (window.BaseStore?.user && typeof window.BaseStore.user.get === 'function') {
            const u = window.BaseStore.user.get();
            compactMode = Boolean(u?.prefs?.compactSidebar);
          }
        } catch (_) {}
        out.preview.compactMode = compactMode;

        const accentPreset = String(prefs?.theme?.accentPreset || prefs?.theme?.accent_preset || '').trim().toLowerCase();
        if (accentPreset) out.theme.accentPreset = accentPreset;
        ...
        const brand = {};
        if (reportLogoDataUrl && reportLogoDataUrl !== companyReportLogo) {
          brand.reportLogoDataUrl = reportLogoDataUrl;
        }
        if (faviconDataUrl && faviconDataUrl !== companyFav && faviconDataUrl !== (brand.reportLogoDataUrl || '')) {
          brand.faviconDataUrl = faviconDataUrl;
        }
      ...
      const payload = {
        ...
        layout: lastExec.layout || '',
        splitData: (lastExec.layout === 'cpcr_split' && lastExec.splitData) ? lastExec.splitData : null,
