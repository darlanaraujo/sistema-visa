...
function lot_public_sheet_text(mixed $value, string $fallback = 'Não informado'): string {
  $text = trim((string)($value ?? ''));
  return $text !== '' ? $text : $fallback;
}

function lot_public_sheet_extract_sinistro(array $lote): string {
...
function lot_public_sheet_collect_gallery(array $itens, array $anexosProdutos, int $loteId, string $token): array {
  $gallery = [];
...
$gallery = lot_public_sheet_collect_gallery($itensDisponiveis, $anexosProdutos, $loteId, $token);
$cover = $gallery[0] ?? null;
$sinistro = lot_public_sheet_extract_sinistro($lote);
...
$title = lot_public_sheet_text($lote['tituloLote'] ?? '', 'Lote disponível') . ' | Impressão da ficha';
$bodyClass = 'lot-public-body lot-public-body--sheet';
$lotPublicMode = 'sheet';
$selectedPublicFichaPrintUrl = lot_public_sheet_print_url($loteId, $token);
$selectedPublicListPrintUrl = lot_public_print_url($loteId, $token);
$extra_css = [
...
$contentFile = __DIR__ . '/lote_publico_ficha_print_content.php';
